...
const CatGrooming = ({ initialActive = 'All Grooming' }) => {
  ...
  const routeMap = { 'All Grooming': '/cats/cat-grooming/all-grooming', 'Brushes & Combs': '/cats/cat-grooming/brushes-combs', 'Dry Bath, Wipes & Perfume': '/cats/cat-grooming/dry-bath', 'Oral Care': '/cats/cat-grooming/oral-care' };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (err) { console.error('Failed to load cat grooming', err); setProducts([]); }
    };
    load();
  ...
  return (
    <>
      <Helmet><title>Shop for Cats — Grooming</title></Helmet>
