...
const sampleProducts = [
  { id: 'cc1', name: 'Nylon Cat Collar', image: '/assets/images/essential/meowsi.webp', badges: ['Adjustable'], variants: ['Small','Medium'], price: 249 },
  { id: 'cc2', name: 'Reflective Harness Set', image: '/assets/images/essential/whiskas.webp', badges: ['Safety'], variants: ['S','M','L'], price: 799 },
  { id: 'cc3', name: 'Custom Name Tag', image: '/assets/images/essential/sheba.webp', badges: ['Personalize'], variants: ['One Size'], price: 199 }
];
...
const CatCollars = ({ initialActive = 'All Collars & Accessories' }) => {
  ...
  const routeMap = { 'All Collars & Accessories': '/cats/cat-collars/all-collars-accessories', 'Collars': '/cats/cat-collars/collars', 'Leash & Harness Set': '/cats/cat-collars/leash-harness', 'Name Tags': '/cats/cat-collars/name-tags' };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (err) { console.error('Failed to load cat collars', err); setProducts([]); }
    };
    load();
  ...
  useEffect(() => {
    ...
    const finalTarget = routeTarget || target || 'cat-collars';
    ...
    let working = products.filter(p => {
      ...
      return c.includes('cat') || sc.includes('cat') || c === finalTarget || sc === finalTarget || String(p.name || '').toLowerCase().includes('collar') || String(p.name || '').toLowerCase().includes('leash') || String(p.name || '').toLowerCase().includes('tag');
    });
  ...
  return (
    <>
      <Helmet><title>Shop for Cats — Collars & Accessories</title></Helmet>
