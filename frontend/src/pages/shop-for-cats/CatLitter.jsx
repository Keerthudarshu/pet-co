...
const CatLitter = ({ initialActive = 'All Litter & Supplies' }) => {
  ...
  const routeMap = { 'All Litter & Supplies': '/cats/cat-litter/all-litter-supplies', 'Litter': '/cats/cat-litter/litter', 'Litter Trays': '/cats/cat-litter/litter-trays', 'Scooper': '/cats/cat-litter/scooper' };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (err) { console.error('Failed to load cat litter', err); setProducts([]); }
    };
    load();
  ...
  useEffect(() => {
    ...
    const finalTarget = routeTarget || target || 'cat-litter';
    ...
    let working = products.filter(p => {
      ...
      return c.includes('cat') || sc.includes('cat') || c === finalTarget || sc === finalTarget || 
             String(p.name || '').toLowerCase().includes('litter') || 
             String(p.name || '').toLowerCase().includes('tray') ||
             String(p.name || '').toLowerCase().includes('scoop');
    });
    if (search) { 
      const t = norm(search); 
          ...
          <div ref={drawerContentRef} className="px-4 pt-4 pb-32 hide-scrollbar overflow-y-auto" style={{ maxHeight: 'calc(100vh - 140px)' }}>
            ...
            <section ref={el => sectionRefs.current['Special Diet'] = el} className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">{specialDiets.map(s=> (
                <button key={s} onClick={() => toggleFilter('specialDiets', s)} 
