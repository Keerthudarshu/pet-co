...
const categories = [
  { id: 'all', label: 'All Cat Toys', img: '/assets/images/cat/ct1.webp' },
  { id: 'catnip', label: 'Catnip Toys', img: '/assets/images/cat/ct2.webp' },
  { id: 'interactive', label: 'Interactive Toys', img: '/assets/images/cat/ct3.webp' },
  { id: 'plush', label: 'Plush Toys', img: '/assets/images/cat/ct4.webp' }
];
...
const CatToys = ({ initialActive = 'All Cat Toys' }) => {
  ...
  const routeMap = { 'All Cat Toys': '/cats/cat-toys/all-cat-toys', 'Catnip Toys': '/cats/cat-toys/catnip-toys', 'Interactive Toys': '/cats/cat-toys/interactive-toys', 'Plush Toys': '/cats/cat-toys/plush-toys' };
  ...
  const subCategories = ['Catnip Toys','Interactive Toys','Plush Toys','Feather Toys','Ball Toys','Mouse Toys'];
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch (err) { console.error('Failed to load cat toys', err); setProducts([]); }
    };
    load();
  ...
  useEffect(() => {
    ...
    const finalTarget = routeTarget || target || 'cat-toys';
    ...
    let working = products.filter(p => {
      ...
      return c.includes('cat') || sc.includes('cat') || c === finalTarget || sc === finalTarget || String(p.name || '').toLowerCase().includes('toy');
    });
    if (search) { 
      const t = norm(search); 
